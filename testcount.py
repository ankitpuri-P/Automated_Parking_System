...
#        print(row)
    ...
    count_text = f"Car's Count: {o}"
    text_size = cv2.getTextSize(count_text, cv2.FONT_HERSHEY_PLAIN, 3, 2)[0]
    text_x = frame.shape[1] - text_size[0] - 23  # Adjust the X-coordinate for right alignment
    cv2.putText(frame, count_text, (text_x, 30), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 2)

    cv2.putText(frame,f"Slots-{space}",(23,30),cv2.FONT_HERSHEY_PLAIN,3,(0,0,255),2)
